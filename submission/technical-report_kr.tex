...
% English source SHA-256: 5a4227d988107c2673c8dff4509ef8ac8b719be53e4bc5c061bcc740518627bc
...
\IfFontExistsTF{D2Coding}{
  ...
  \setCJKmonofont[Path=../tmp/pdfs/fonts/]{NotoSansKR-Regular.ttf}
}
...
후에는 복합 고위험 초안만 선택적으로 제한된 verifier 경로에 들어간다. 명시적
risk sidecar와 mock verifier가 이 경로를 로컬에서 실행하고, native 검증은 별도
Root orchestration으로 유지한다. 10편
...
상태 전이를 명시적 risk sidecar와 결정론적 mock verifier로 DRY-RUN에서
실행한다. Native 검증은 별도 Root 소유 Codex 작업이며, 이는 control flow를
검증할 뿐 unseen paper의 품질 향상을 입증하지 않는다.
...
포함된 \texttt{run\_batch.py}는 manifest 동결용 \texttt{BUILD}와 mock-only
\texttt{DRY-RUN}을 지원한다. 명시적 \texttt{--calibration mock}과 sidecar로
verifier gate와 repair를 실행하며, output 생성 전에 \texttt{LIVE}를 거부한다.
승인된 live 시작 시점에는 Skill이 최대 45초의 read-only contract
...
Post-build evaluator는 70개 regression test를 모두 통과했고,
installed/staged package의 정확한 일치, 제한된 calibration 경로의 실행,
정상 합성 모의실험의 30/30 유효·duplicate-free 완료를 확인했다. Native
Worker instruction surface는
...
보고서는 endpoint나 selector를 가정하지 않는다. Mock verifier는 risk, gate,
PASS/REPAIR, 공유 repair, 재검증, 감사 상태 전이를 실행하지만 독립적인 과학
판정자는 아니다. Native 검증은 Root가 별도로 orchestration하며 unseen-paper
품질과 inter-reviewer agreement에 미치는 효과는 측정하지 않았다.
